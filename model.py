...
class DCGAN(object):
  ...
  def train(self, config):
    d_optim = npu_tf_optimizer(tf.train.AdamOptimizer(config.learning_rate, beta1=config.beta1) \
              ).minimize(self.d_loss, var_list=self.d_vars)
    g_optim = npu_tf_optimizer(tf.train.AdamOptimizer(config.learning_rate, beta1=config.beta1) \
      ...
      for idx in xrange(0, int(batch_idxs)):
        ...
        if np.mod(counter, config.sample_freq) == 0:
          if config.dataset == 'mnist':
            ...
            mox.file.copy_parallel(src_url='{}/train_{:08d}.png'.format(config.sample_dir, counter), dst_url="obs://chenxubin/dcgan/celebA_sample/"+'train_{:08d}.png'.format(counter))
