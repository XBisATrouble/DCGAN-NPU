...
class DCGAN(object):
    ...
    def train(self, config):
        d_optim = npu_tf_optimizer(tf.train.AdamOptimizer(config.learning_rate, beta1=config.beta1)).minimize(
            self.d_loss, var_list=self.d_vars)
        g_optim = npu_tf_optimizer(tf.train.AdamOptimizer(config.learning_rate, beta1=config.beta1)).minimize(
